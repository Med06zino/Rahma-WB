...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleScrollAnimation() {
        const elements = document.querySelectorAll(
            '.section-title, .ornament, .about-text, .about-image, .activity-card, .news-card, .info-item, .contact-form'
        );

        elements.forEach(el => {
            if (isInViewport(el)) {
                if (el.classList.contains('about-text') || el.classList.contains('info-item')) {
    ...
    window.addEventListener('scroll', () => {
        header.style.backgroundColor =
            window.scrollY > 50 ? 'rgba(255,255,255,1)' : 'rgba(255,255,255,0.9)';
    ...
    contactForm?.addEventListener('submit', e => {
        e.preventDefault();
        const msg = `Bonjour, je vous contacte depuis le site Dahira Rahma.%0A
Nom : ${name.value}%0A
Email : ${email.value}%0A
Sujet : ${subject.value}%0A
Message : ${message.value}`;

        window.open(`https://wa.me/221775419501?text=${msg}`, '_blank');
        contactForm.reset();
    });
